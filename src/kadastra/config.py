...
class Settings(BaseSettings):
    ...
    gar_lookup_cadnum_index_path: Path = Path(
        "data/silver/gar_lookup/cadnum_index.parquet"
    )
    gar_lookup_mun_lookup_path: Path = Path(
        "data/silver/gar_lookup/mun_lookup.parquet"
    )
    # Pivoted PARAMS lookup keyed on objectid: oktmo_full / okato /
    # postal_index. Built alongside cadnum_index (same XMLs).
    gar_lookup_object_params_path: Path = Path(
        "data/silver/gar_lookup/object_params.parquet"
    )
