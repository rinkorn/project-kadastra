...
def compute_object_neighbor_features(objects: pl.DataFrame, radius_m: float) -> pl.DataFrame:
    column_names = [_column_name(cls) for cls in _TRACKED_CLASSES]

    if objects.is_empty():
        return objects.with_columns([pl.lit(None, dtype=pl.Int64).alias(col) for col in column_names])
    ...
    cells = [h3.latlng_to_cell(float(obj_coords[i, 0]), float(obj_coords[i, 1]), _BUCKET_RES) for i in range(n)]
    ...
    counts: dict[AssetClass, np.ndarray] = {cls: np.zeros(n, dtype=np.int64) for cls in _TRACKED_CLASSES}
    ...
    return objects.with_columns([pl.Series(_column_name(cls), counts[cls]) for cls in _TRACKED_CLASSES])
