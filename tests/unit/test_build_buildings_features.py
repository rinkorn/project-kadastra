...
def _buildings_csv() -> bytes:
    df = pl.DataFrame(
        {
            "lat": [KAZAN_LAT, KAZAN_LAT],
...
) -> BuildBuildingsFeatures:
    return BuildBuildingsFeatures(
        coverage_reader=coverage_reader,
        raw_data=raw_data,
        feature_store=feature_store,
        buildings_key="buildings/kazan.csv",
    )


def test_execute_reads_buildings_from_raw_data() -> None:
    ...
    assert "buildings/kazan.csv" in raw_data.reads
...
def test_execute_does_not_re_read_csv_per_resolution() -> None:
    ...
    assert raw_data.reads.count("buildings/kazan.csv") == 1
