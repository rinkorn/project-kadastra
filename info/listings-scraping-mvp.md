...
Следующие шаги (пересмотрено 2026-08-21 по разведке пригодности, см.
[ADR-0031](decisions/0031-listings-as-target.md)): Avito маргинален
(196 строк, 0 % coords) — доскачка деприоритизирована; приоритет —
использование CIAN как market-target для apartment; Иркутск — после
расширения скоупа.
...
- **Yandex** — средняя позиция: координат нет (в raw HTML их вообще
нет — подгружает отдельный maps-API, который не сохранялся), но
`raw_text` карточки содержит **полный адрес с ЖК и улицей** —
гипотетически геокодабельно через Nominatim (качество адресных строк
не измерено, см. разведку 2026-08-21 в ADR-0031). Стабильно проходит
до ~99 страниц.
