...
[ADR-0007](decisions/0007-kazan-agglomeration-scope.md)). Иркутск
запланирован вторым после расширения скоупа.
...
1. **Download** — [scripts/download_listings_mvp.py](../scripts/download_listings_mvp.py)
   ...
   * Пример: `uv run --with patchright python scripts/download_listings_mvp.py
     --n-pages 500 --cities kazan --sources yandex_realty cian`.
...
* **CIAN** п. 6.7 — запрет автоматизированной обработки.
* **Avito** п. 7.1.5 — запрет автоматизированного сбора.
* **Yandex Realty** — запрет на использование в коммерческих целях.

MVP-выгрузка — **исследовательская**, для выбора партнёра. Production-cron
требует один из:

1. Партнёрский API одной из площадок (по итогам MVP-сравнения).
2. Открытые научные датасеты (нет на 2026-04 для российских квартир).
3. Договор с Росреестром/ППК на выгрузку фактических сделок (target —
   лучше листингов, но нужен закрытый канал).
