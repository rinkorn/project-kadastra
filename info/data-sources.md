...
| `nspd/zouit-tatarstan/page-NNNN.json` | **ЗОУИТ Татарстана (NSPD L36302)**, фильтр `options.reg_numb_border ⊂ "16:"` | 889 страниц = **177 663 зоны**, ~1.7 ГБ (тот же [scripts/download_nspd_layer.py](../scripts/download_nspd_layer.py), фильтр через `--filter-json`). Атрибуты: `options.type_zone` (вид зоны), `reg_numb_border`, документы; геометрия Polygon/MultiPolygon. Источник ЗОУИТ-фич [ADR-0025](decisions/0025-misc-enrichment-cbd-heritage-zoouit.md). |
| `dem/Copernicus_DSM_*.tif` | **Copernicus GLO-30 DEM**, 4 тайла (N55/N56 × E048/E049) | ~115 МБ. 30 м, open license (© DLR). Заменил SRTM из исходной спеки — см. [ADR-0023](decisions/0023-topography-dem-features.md). |
| `osm/kazan-agg-minor_roads.geojsonseq` | **Minor roads Казанской агломерации** (residential/service/pedestrian/footway/living_street/unclassified) | 86 685 features (ноды + ways), 22 МБ. Way-классы предраспарсены в `kazan-agg-minor_road_ways.parquet` (69 827 ways). Источник для [ADR-0024](decisions/0024-road-network-advanced-features.md) — существующий граф хранит только major-классы. |
| `osm/kazan-agg-heritage.geojsonseq` | **Объекты культурного наследия** (OSM, тег `heritage=*`) | 285 строк, ~188 ОКН с полигонами, `ref:egrokn` у 60. Fallback-источник вместо недоступного `opendata.mkrf.ru` — см. [ADR-0025](decisions/0025-misc-enrichment-cbd-heritage-zoouit.md). |
...
| `emiss/31452/raw/raw_{YYYY-MM-DD}.xls` | **ЕМИСС / fedstat.ru, индикатор #31452** — «Средняя цена 1 кв.м. общей площади квартир на рынке жилья» (по регионам в целом, **без разреза на города-центры**). Pivot-таблица .xls (3 991 строк × 31 колонка), **CC-BY 3.0**. Покрытие: **2000-2025 (26 лет)**, 114 регионов/округов, квартально (I-IV), 2 рынка (первичный/вторичный), 5 типов квартир (низкое/среднее/улучшенное/элитное/все). | Скачивается через браузер (patchright): curl отдаёт SPA-оболочку, реальный xls уходит POST'ом на `data.do?format=excel&id=31452` после JS-инициализации фильтров. Прим.: тот же приказ Росстата №572 от 12.08.2022 (форма 1-РЖ), но индикатор-«ствол» — длинный исторический ряд региональных средних. |
...
| `emiss/61781/raw/raw_{YYYY-MM-DD}.xls` | **ЕМИСС / fedstat.ru, индикатор #61781** — «Средняя цена 1 кв.м. общей площади квартир на рынке жилья **по центрам субъектов РФ**» (т.е. _только_ административный центр субъекта, например Казань для Татарстана, Иркутск для Иркутской обл.). Pivot-таблица .xls (2 477 × 11), **CC-BY 3.0**. Покрытие: **2021-2025 (5 лет)**, 80 центров субъектов, квартально, 2 рынка, 5 типов квартир. | Скачивается через браузер (patchright), аналогично #31452 — curl больше не работает. Молодая методология: приказ Росстата №572 от 12.08.2022 (форма 1-РЖ), city-level разрез появился именно с этой ревизии. |
